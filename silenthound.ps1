...
function Dump-LDAP {
    ...
    try {
        ...
        $connection = New-Object System.DirectoryServices.Protocols.LdapConnection($server, $credential, $authType)
...
function Extract-All {
    ...
    foreach ($row in $Dump) {
        ...
        if ($Groups -or $All -and $row.Attributes["objectClass"] -contains "group") {
            $GroupUserDictList += [PSCustomObject]@{ Group = $row.Dn; Members = $row.Attributes["member"] }
        }

        if ($OrgUnit -or $All -and $row.Attributes["objectClass"] -contains "organizationalUnit") {
            $OU_List += $row.Dn
        }
